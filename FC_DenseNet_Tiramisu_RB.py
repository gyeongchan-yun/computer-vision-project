...
def ResidualBlock(stack, depth_bottleneck, scope=None):
  with tf.name_scope(scope) as sc:
    preact = tf.nn.relu(slim.batch_norm(stack))
    shortcut = slim.conv2d(preact, depth_bottleneck, [1, 1], stride=1)
    residual = slim.conv2d(preact, depth_bottleneck, [1, 1], stride=1)
    residual = slim.conv2d(residual, depth_bottleneck, [3, 3], stride=1)
    residual = slim.conv2d(residual, depth_bottleneck, [1, 1], stride=1)
    stack = shortcut + residual
    return stack
...
def TransitionDown(inputs, n_filters, dropout_p=0.2, scope=None):
  ...
  """
  with tf.name_scope(scope) as sc:
    l = ResidualBlock(inputs, n_filters)
...
def build_fc_densenet(inputs, preset_model='FC-DenseNet56', num_classes=12, n_filters_first_conv=48, n_pool=5, growth_rate=12, n_layers_per_block=4, dropout_p=0.2, scope=None):
    ...
    with tf.variable_scope(scope, preset_model, [inputs]) as sc:
      ...
      stack = ResidualBlock(stack, growth_rate, scope='residualblock%d' % (i + 1))
